...
class RPCFRPlusSolver(_CFRSolverBase):
    def __init__(self, game, lam=0.6, beta=0.3, gamma=5):
        super().__init__(
            game,
        ...
        self._beta = beta    # EMA coefficient: 0~1, the larger, the newer
        self._gamma = gamma  # Strategy average weighting (maintaining long-term stability)
    ...
    def evaluate_and_update_policy(self):
        ...
        for player in range(self._game.num_players()):
            ...
            # CFR+ accumulated regret for updates：regrets = max(regrets + imm_regret, 0)
            for node in self._info_state_nodes.values():
                for a in node.legal_actions:
                    node.cumulative_regret[a] = max(
                        node.cumulative_regret[a] + node.imm_regret[a], 0
                    )

            # Using a prediction regret update strategy
            self._update_current_policy_with_prediction()

            # Clear instant regrets
            for node in self._info_state_nodes.values():
    ...
    def _update_current_policy_with_prediction(self):
        """Use Predicted Regrets (EMA based on imm.regret) to update policies.
                Retain the cumulative regret updates and non negative truncation of CFR+.
        """
        for info_state, node in self._info_state_nodes.items():
            # 1) Initialization: Maintain a predictive signal table (EMA) for each information state
            if not hasattr(node, 'pred_signal'):
                node.pred_signal = collections.defaultdict(float)

            # 2) Update EMA with immediate regret (imm_regret) to obtain the predicted signal φ _t (a)
            #    φ_t(a) = (1-β)*φ_{t-1}(a) + β*imm_regret_t(a)
            for a in node.legal_actions:
                imm = node.imm_regret.get(a, 0.0)
                node.pred_signal[a] = (1.0 - self._beta) * node.pred_signal[a] + self._beta * imm

            # 3) Retrieve and overwrite the current policy distribution of the information state (this is a variable dictionary of actions → probabilities)
            state_policy = self._current_policy.policy_for_key(info_state)

            # 4) Predicting Regrets： R_t(a) = max( (1-λ) R_t(a) + λ φ_t(a), 0 )
            pred_regrets = {}
            for a in node.legal_actions:
            ...
            # 5) regret-matching+   Based on prediction regret, if the sum is 0, it degenerates into a uniform strategy
